...
async function sendEmail(page, to, subject, body) {
  ...
  await bodyField.type(body, { delay: 10 });
  await sleep(500);

  const sendBtn = await page.waitForSelector('[data-tooltip="Envoyer"]', { timeout: 10000 }).catch(() =>
    page.waitForSelector('[data-tooltip="Send"]', { timeout: 5000 })
  );
  await sendBtn.click();
  await sleep(1500);
}
